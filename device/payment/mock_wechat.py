...
class MockWeChatPayProvider(PaymentProvider):
    """Mock WeChat Pay provider for testing and demo"""
    
    def __init__(self):
        self.active_payments = {}  # order_id -> payment_data
        self.test_mode = True
        self.auto_pay_delay = 10  # Auto-pay after 10 seconds in test mode
    
    async def create_qr(self, amount: float, order_id: str, expires_in_s: int = 300) -> PaymentQR:
        """Create QR code for WeChat payment"""
        expire_time = utc_now() + timedelta(seconds=expires_in_s)
        
        # Create QR data (mock format)
        qr_data = f"WECHAT:ORDER:{order_id}:AMOUNT:{amount:.2f}"
        ...
        logger.info(f"Created WeChat payment QR for order {order_id}, amount: ¥{amount:.2f}")
    ...
    async def cancel(self, order_id: str) -> Dict[str, Any]:
        ...
        if order_id in self.active_payments:
            ...
            logger.info(f"Canceled WeChat payment for order {order_id}")
            return {"ok": True}
    ...
    async def _auto_complete_payment(self, order_id: str):
        ...
        if order_id in self.active_payments:
            ...
            if payment["status"] == "pending":
                payment["status"] = "paid"
                payment["txn_id"] = f"wx_txn_{order_id}_{int(utc_now().timestamp())}"
                logger.info(f"Auto-completed WeChat payment for order {order_id}")
    ...
    def simulate_payment_success(self, order_id: str) -> bool:
        ...
        if order_id in self.active_payments:
            ...
            if payment["status"] == "pending":
                payment["status"] = "paid"
                payment["txn_id"] = f"wx_manual_{order_id}_{int(utc_now().timestamp())}"
                logger.info(f"Manually completed WeChat payment for order {order_id}")
                return True
        return False
    ...
    def simulate_payment_failure(self, order_id: str, reason: str = "Payment failed") -> bool:
        ...
        if order_id in self.active_payments:
            ...
            if payment["status"] == "pending":
                ...
                logger.info(f"Manually failed WeChat payment for order {order_id}: {reason}")
                return True
        return False
    ...
    def clear_expired_payments(self):
        ...
        if expired_orders:
            logger.info(f"Marked {len(expired_orders)} payments as expired")

# Global mock WeChat Pay provider
mock_wechat = MockWeChatPayProvider()